...
![](_assets/Pasted%20image%2020260617083800.png)
...
![](_assets/Pasted%20image%2020260617084402.png)
...
### 💻 Kiểm chứng PE 02: Classic Code Injection Remote (PIC - RWX)

- **Bản chất kỹ thuật:** Malware cấp phát hai phân vùng độc lập từ xa bên trong Notepad: một phân vùng mang đặc quyền `RWX` chứa thân hàm `LaunchCalculator` và một phân vùng `RW` chứa tham số `THREAD_DATA`.

- **Quy trình kiểm tra bằng System Informer:**

1. Mở sẵn `notepad.exe`. Chạy Malware PE 02 $\rightarrow$ Cửa sổ `calc.exe` bật lên và Malware hiện hộp thoại tạm dừng.

![](_assets/Pasted%20image%2020260617091001.png)

2. Vào System Informer $\rightarrow$ Nhấn đúp tiến trình **`notepad.exe`** $\rightarrow$ Tab **Memory**.

![](_assets/Pasted%20image%2020260617091029.png)

![](_assets/Pasted%20image%2020260617091041.png)

3. **Chỉ dấu đúng bản chất:**

- Tìm dòng mang thuộc tính **`Type: Private`** và **`Protection: RWX`**. Nhấn đúp vào dòng này, chọn tab **Hex** $\rightarrow$ sẽ nhìn thấy mã máy thô của hàm PIC.

![](_assets/Pasted%20image%2020260617091117.png)

![](_assets/Pasted%20image%2020260617091142.png)

- Tìm dòng mang thuộc tính **`Type: Private`** và **`Protection: RW`** nằm ngay lân cận. Nhấn đúp chọn tab **Hex** $\rightarrow$ Vinh sẽ nhìn thấy chuỗi lệnh rõ ràng dạng text phẳng: `cmd.exe /c start calc`.

![](_assets/Pasted%20image%2020260617091200.png)

![](_assets/Pasted%20image%2020260617091213.png)

### Demo
![](_assets/devenv_rKNHXykY0I.gif)
