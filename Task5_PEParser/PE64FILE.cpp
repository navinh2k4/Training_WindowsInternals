...
void PE64FILE::ParseSectionHeaders() {
    ...
    for (int i = 0; i < (int)PEFILE_NT_HEADERS_FILE_HEADER_NUMBER0F_SECTIONS; i++) {
        int offset = (PEFILE_DOS_HEADER.e_lfanew + sizeof(DWORD) + sizeof(IMAGE_FILE_HEADER) + PEFILE_NT_HEADERS_FILE_HEADER_SIZEOF_OPTIONAL_HEADER) + (i * sizeof(IMAGE_SECTION_HEADER));
...
// Sửa dứt điểm lỗi Typo tên Class thừa chữ FILE
...
void PE64FILE::PrintNTHeadersInfo() {
    ...
    printf("\n  Data Directories:\n");
    printf("    * Export Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_EXPORT_DIRECTORY.VirtualAddress, PEFILE_EXPORT_DIRECTORY.Size);
    printf("    * Import Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_IMPORT_DIRECTORY.VirtualAddress, PEFILE_IMPORT_DIRECTORY.Size);
    printf("    * Resource Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_RESOURCE_DIRECTORY.VirtualAddress, PEFILE_RESOURCE_DIRECTORY.Size);
    printf("    * Exception Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_EXCEPTION_DIRECTORY.VirtualAddress, PEFILE_EXCEPTION_DIRECTORY.Size);
    printf("    * Security Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_SECURITY_DIRECTORY.VirtualAddress, PEFILE_SECURITY_DIRECTORY.Size);
    printf("    * Base Relocation Table: RVA: 0x%X | Size: 0x%X\n", PEFILE_BASERELOC_DIRECTORY.VirtualAddress, PEFILE_BASERELOC_DIRECTORY.Size);
    printf("    * Debug Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_DEBUG_DIRECTORY.VirtualAddress, PEFILE_DEBUG_DIRECTORY.Size);
    printf("    * Architecture Specific Data: RVA: 0x%X | Size: 0x%X\n", PEFILE_ARCHITECTURE_DIRECTORY.VirtualAddress, PEFILE_ARCHITECTURE_DIRECTORY.Size);
    printf("    * RVA of GlobalPtr: RVA: 0x%X | Size: 0x%X\n", PEFILE_GLOBALPTR_DIRECTORY.VirtualAddress, PEFILE_GLOBALPTR_DIRECTORY.Size);
    printf("    * TLS Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_TLS_DIRECTORY.VirtualAddress, PEFILE_TLS_DIRECTORY.Size);
    printf("    * Load Configuration Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_LOAD_CONFIG_DIRECTORY.VirtualAddress, PEFILE_LOAD_CONFIG_DIRECTORY.Size);
    printf("    * Bound Import Directory: RVA: 0x%X | Size: 0x%X\n", PEFILE_BOUND_IMPORT_DIRECTORY.VirtualAddress, PEFILE_BOUND_IMPORT_DIRECTORY.Size);
    printf("    * Import Address Table: RVA: 0x%X | Size: 0x%X\n", PEFILE_IAT_DIRECTORY.VirtualAddress, PEFILE_IAT_DIRECTORY.Size);
    printf("    * Delay Load Import Descriptors: RVA: 0x%X | Size: 0x%X\n", PEFILE_DELAY_IMPORT_DIRECTORY.VirtualAddress, PEFILE_DELAY_IMPORT_DIRECTORY.Size);
    printf("    * COM Runtime Descriptor: RVA: 0x%X | Size: 0x%X\n", PEFILE_COM_DESCRIPTOR_DIRECTORY.VirtualAddress, PEFILE_COM_DESCRIPTOR_DIRECTORY.Size);
    printf(" ----------------------------------\n");
}
...
void PE64FILE::PrintImportTableInfo() {
    ...
    for (int i = 0; i < _import_directory_count; i++) {
        ...
        if (PEFILE_IMPORT_TABLE[i].TimeDateStamp == 0) printf("       Bound: FALSE\n");
        ...
        while (true) {
            ILT_ENTRY_64 rawEntry64;
            fseek(Ppefile, (ILTAddr + (entrycounter * sizeof(ULONGLONG))), SEEK_SET);
            fread(&rawEntry64, sizeof(ULONGLONG), 1, Ppefile);

            // IMAGE_ORDINAL_FLAG64 is 0x8000000000000000
            bool isOrdinal = (rawEntry64 & 0x8000000000000000ULL) != 0;
            DWORD HintRVA = 0x0;
            WORD ordinal = 0x0;
            if (!isOrdinal) HintRVA = (DWORD)(rawEntry64 & 0x7FFFFFFFFFFFFFFFULL);
            else ordinal = (WORD)(rawEntry64 & 0xFFFF);
